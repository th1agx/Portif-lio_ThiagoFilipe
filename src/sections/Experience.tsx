...
export function Experience({ t }: { t: Translation }) {
  ...
  return (
    <section id="experiencia" className="px-4 py-24 sm:px-6 lg:px-8" ref={containerRef}>
      <div className="mx-auto max-w-7xl">
        ...
        <div className="border-t border-vanilla-border" ref={listRef}>
          {t.experience.items.map((item) => (
            <article key={`${item.role}-${item.period}`} className="grid gap-6 md:grid-cols-[1fr_3fr] border-b border-vanilla-border py-12 will-change-transform">
              <div>
