...
export function LoadingScreen({ onComplete }: LoadingScreenProps) {
  ...
  useEffect(() => {
    ...
    // 7. Curtain rises (black background fades into transparent, revealing the scene)
  ...
  return (
    <div ref={containerRef} className="fixed inset-0 z-[100] pointer-events-none">
      ...
      {/* Black Overlay */}
      <div ref={overlayRef} className="absolute inset-0 bg-black flex flex-col items-center justify-center text-[#F4F1EA]">
        ...
        <div 
          ...
          style={{
            backgroundImage: 'linear-gradient(to right, rgba(244, 241, 234, 0.5) 1px, transparent 1px), linear-gradient(to bottom, rgba(244, 241, 234, 0.5) 1px, transparent 1px)',
        ...
        <div ref={progressContainerRef} className="absolute bottom-24 left-1/2 -translate-x-1/2 flex flex-col items-center gap-4 w-64 opacity-0 will-change-transform">
          <div className="w-full h-[1px] bg-[#F4F1EA]/10 relative overflow-hidden">
            <div ref={progressRef} className="absolute left-0 top-0 bottom-0 bg-[#F4F1EA]/80 w-0" />
          </div>
